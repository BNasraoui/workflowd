...
A WorkflowDefinition that enables Structure MUST also enable Design in its fixed linear
order. Structure cannot be the first configured stage and cannot activate from a skipped,
generic, or merely accepted Design revision; only the matching specialized Design
StageRun success after confirmed promotion and snapshot releases it. A definition that
could conditionally exclude Design while retaining Structure is invalid.
...
| Stage      | From             | Event                                                                     | To                                                         |
| ---------- | ---------------- | ------------------------------------------------------------------------- | ---------------------------------------------------------- |
| any        | `blocked`        | prior stage succeeds or this is first                                     | `active`                                                   |
| non-Design | `blocked`        | condition excludes stage                                                  | `skipped`                                                  |
| non-Design | `active`         | current revision publishes and no review/gate is required                 | `succeeded`                                                |
| non-Design | `active`         | current revision publishes and review is required                         | `waiting_review`                                           |
| non-Design | `waiting_review` | synthesis accepts and no gate remains                                     | `succeeded`                                                |
| non-Design | `waiting_review` | synthesis requests revision                                               | `active` with next revision                                |
| non-Design | `waiting_review` | synthesis asks human                                                      | `waiting_human`                                            |
| non-Design | `waiting_human`  | approve                                                                   | policy-selected `succeeded`, `active`, or `waiting_review` |
| non-Design | `waiting_human`  | request changes                                                           | `active` with next revision                                |
| non-Design | `waiting_human`  | approve product change                                                    | `waiting_ticket`                                           |
| Design     | `active`         | specialized acceptance requires a ready changed ticket                    | `waiting_ticket`                                           |
| Design     | `active`         | exact accepted promotion and graph snapshot are authoritatively confirmed | `succeeded`                                                |
| any        | `waiting_ticket` | ready changed ticket arrives                                              | `superseded` with Generation                               |
| non-Design | nonterminal      | required child operation fails by declared effect                         | `failed` or `waiting_human`                                |
| Design     | `active`         | required child needs human/operator intervention                          | remain `active`; child operation or gate owns the wait     |
| any        | nonterminal      | required child fails terminally                                           | `failed`                                                   |
| any        | nonterminal      | cancellation                                                              | `cancelled`                                                |
| any        | nonterminal      | newer Generation                                                          | `superseded`                                               |

A succeeded or skipped stage releases only the next configured stage.

Design is never eligible for a generic publication, review-synthesis, gate-approval, or
skip transition. Every configured Design StageRun MUST use the specialized acceptance
subflow: semantic-ownership review, independent impact-and-risk analysis, a human gate
over the complete acceptance package, exact Provenance promotion, and an authoritatively
confirmed graph snapshot. Human approval only accepts its revision; only the final Design
transition above can succeed the StageRun and release Structure.
Design remains `active` while its specialized reviews, package gate, promotion, or child
operation waits; those child records own their waiting states, so no generic
`waiting_human` approval can accept or succeed Design.
...
A non-Design document publication with no review or gate sets published and accepted
pointers together. Otherwise, for a non-Design document, publication sets published and
review or gate sets accepted. Design publication only sets `publishedRevision`; the
specialized complete-package human approval is the sole event that may set its
`acceptedRevision`, and that accepted pointer does not itself succeed Design.
...
structurePolicyRevision
structurePolicySha256
```
...
selection and graph-snapshot acceptance. Before workflowd-vs3.4 publishes the Design
revision, it also pins the Structure policy revision and hash from the Structure
`StageDefinition` selected by the already-hashed WorkflowDefinition. That policy covers
Structure classification, authority, coverage, and exit rules. All three policy identities
are part of the acceptance scope carried by every report, package, response, promotion
request, promotion result, and accepted graph snapshot. None can be selected or changed
after review or approval. A report that matches the bytes but not the Generation, source
set, or any policy revision is stale.
...
Generation, and all three policy revisions. Eligible promoted meaning is limited to selected
...
semantic-record identities and versions. The result and snapshot metadata both bind the
exact `DesignAcceptanceScope`, including its Structure policy identity. Missing,
conflicting, merely attempted, locally inferred, partially observed, or incompletely
selected records do not satisfy the result.
...
structurePolicyRevision and structurePolicySha256 from DesignAcceptanceScope
```

Workflowd-vs3.4 constructs this input only after checking that the package, gate response,
promotion request, promotion result, and snapshot carry the same exact
`DesignAcceptanceScope`. Therefore every reference can be cross-checked for WorkflowId,
Generation, Design artifact, source set, WorkflowDefinition, and Design, promotion, and
Structure policy revisions/hashes; the linked package, gate response, request, result, and
snapshot identities MUST also agree where applicable. The explicit Structure policy
fields in `StructureInput` MUST equal the already-approved scope and are not a late policy
selection. Structure rejects a floating graph head, a reconstructed “latest” package, or
a snapshot not produced by the authoritative result.
...
```text
AcceptNonDesignStageRevision
AcceptDesignPackageAndCreatePromotion
CreateStageRevision
CreateReviewRound
AwaitTicketRevision
RestoreSavedStates
...
`AcceptNonDesignStageRevision` rejects a Design target. The only response effect that may
set Design's `acceptedRevision` is `AcceptDesignPackageAndCreatePromotion`; in one
transaction it validates the exact complete `DesignAcceptancePackage`, authenticated
`GateResponse`, and `DesignAcceptanceScope`, stores the response, sets the accepted
pointer, and creates the deterministic `ProvenancePromotionRequest` described above. It
does not succeed Design. Generic operation-gate resolution may resume a Design child but
cannot accept its revision or release Structure.
