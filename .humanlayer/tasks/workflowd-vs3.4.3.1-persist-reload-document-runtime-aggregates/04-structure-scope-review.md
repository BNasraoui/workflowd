...
One allocation detail remains insufficiently explicit: accepted Design lines 210-214 require a stable owner-crossing identity hook and a publication-operation identity hook. The Structure explicitly names producer/publication ownership but does not identify whether that schema also supplies the owner-crossing hook. This does not prevent sizing, but the shared-layout task must allocate that obligation explicitly before implementation.
...
| --- | --- | --- | ---: | ---: | ---: | --- | --- |
| Install the complete inactive shared runtime layout | None | `src/store/migrations.ts`, `test/store/migrations.test.ts` | 620 | 880 | 1210 | Complete-layout AC; SQL portions of contradictory-state AC; legacy preservation and inactivity AC. C1 tagged relational shape, C5 local SQL enforcement part, C6 append-only format boundary. R1 nullable/inferred-state prevention part and R5 malformed durable-state prevention part. Must explicitly allocate both accepted identity hooks. | Requires its own scope review; not an implementation-ready leaf. |
| Define and atomically round-trip one exact document aggregate | Shared layout | `src/qrspi/stage-runtime.ts`, `src/qrspi/store.ts`, new `test/qrspi/store.test.ts` | 580 | 850 | 1150 | Valid document aggregate scenario; exact guarded pointers, immutable artifacts, and WorkflowOperation ownership; strict input decode; rollback and deterministic reload. C1 document tagged aggregate remainder and C5 semantic identity/hash enforcement part. R1 variant contradiction prevention part and R5 untrusted read prevention part. | Requires its own scope review; not an implementation-ready leaf. |
| Complete contradictory-state diagnostics and inactivity proof | Document round trip | `src/qrspi/store.ts`, new `test/qrspi/store.test.ts`, `test/qrspi/workflow-start.test.ts` | 365 | 570 | 850 | Contradictory tagged-state scenario; exact missing/malformed/duplicate/reordered/tag/role/identity/hash diagnostics; non-mutation; unchanged WorkflowStart and no runtime claim path. C5 trusted-read completion and C6 inactivity proof. R5 corruption exposure prevention remainder. | Requires its own scope review; not an implementation-ready leaf. |
